...
req_pacs <- c("tidyverse", 'readxl', 'scales', 'janitor', 'paletteer', 'VariantAnnotation', "cn.mops", "htmltools", 
              'ggrepel', 'here',  'DT', 'plotly', 'mmod', 'poppr', 'circlize', "downloadthis", "tabulizer", "htmltab/htmltab")

better_p_load <- function(packages){
  safely_p_load <- purrr::safely(pacman::p_load)
  # basename("htmltab/htmltab")
  try_load <- safely_p_load(char=basename(packages), install = FALSE)
  if (!all(try_load$result==TRUE) | !is.null(try_load$error)) pak::pak(packages)
  try_load <- safely_p_load(char=basename(packages), install = FALSE)
}

  
better_p_load(req_pacs)
# pacman::p_load(tidyverse, readxl, scales, janitor, paletteer, VariantAnnotation, cn.mops,
#                htmltools, ggrepel, here,  DT, plotly, mmod, poppr, circlize, downloadthis,
#                tabulizer, htmltab, install = FALSE)
...
cloudstor_links_2018 <- hyperlinks_from_excel(here("sample_info/A_rabiei_isolate_list_for_wgs.xlsx"), 
                                         ExcelSheet = "submission_info") %>% 
                    dplyr::select(target, CloudStor_Folder=text) %>% 
                    distinct()

sequencing_table_2018 <- read_excel(here("sample_info/A_rabiei_isolate_list_for_wgs.xlsx"), 
                               sheet = "submission_info") %>% 
  mutate(Submission_id=sub("_S\\d+$", "", Submission_id)) %>% 
  left_join(cloudstor_links_2018)

cloudstor_links_2020 <- hyperlinks_from_excel(here("sample_info/AGRF_NGS_yield.xlsx"), 
                                         ExcelSheet = "AGRF_NGS_yield") %>% 
                    dplyr::select(target, CloudStor_Folder=text) %>% 
                    distinct()

sequencing_table_2020 <- read_excel(here("sample_info/AGRF_NGS_yield.xlsx"), 
                               sheet = "AGRF_NGS_yield") %>% 
  left_join(cloudstor_links_2020)
sequencing_table <- sequencing_table_2020 %>% 
  dplyr::select(Submission_id=`Sample Name`, Isolate, CloudStor_Folder, target) %>% 
  mutate(Sequencing_Centre="AGRF") %>% 
  bind_rows(sequencing_table_2018 )
  


samples_table <- readxl::read_excel(here("sample_info/Summary_tables_AR_2020.xlsx"), "samples_db") %>% 
  filter(Isolate %in% sequencing_table$Isolate)
  
# 
# 
# samples_table <- selected_samples %>% left_join(patho_table %>% dplyr::select(Isolate, Year, Pathogenicity=Path_rating)) %>% 
#   arrange(Isolate)
...
```{r samples-table, eval=TRUE, results='asis', echo=FALSE, out.width="100%"}
# create table caption hook
DT_caption("samples-table", HTML("<i>Ascochyta rabiei</i> samples used in this study."))

DT_samples_table <- samples_table %>% 
  dplyr::select(-any_of(c("Sample_Id", "Isolate_Code", "Supplied_by", "Delivered_to", "Collection_Date",
                   "Isolate_Usq", "Comment", "Genotyped", "Path_rating2", "lat", "lon"))) %>% 
  relocate(Isolate, .before = 1)
DT_samples_table   %>% 
datatable(rownames = FALSE, colnames = snakecase::to_title_case(colnames(DT_samples_table), abbreviations = c("ICC")),
          extensions = "Buttons",
          # server = FALSE,
          options = list(paging = TRUE,
                           scrollX=TRUE, 
                           pageLength = 10,
                           lengthMenu = c(10, 25, 50, 100, 150, 200, 250),
                           searching = TRUE,
                           ordering = TRUE,
                           dom = 'lfrtipB',
...
# download_this(DT_samples_table %>% clean_names(case="title", abbreviations = c("ICC")),
#                 output_name = "Arab_2020_isolate_metadata",
#               output_extension = ".csv",
#               button_label = "Download table as csv file",
#               button_type = "success",
#               has_icon = TRUE,
#               icon = "fa fa-save",
...
REF_DIR=$HOME/data/reference_genomes/Ascochyta_reference_genomes/ArME14_v2_NCBI # New published genome from NCBI on Griffith HPC
# REF_DIR=$HOME/data/reference_genomes # on Awoonga HPC
# GENOME="$REF_DIR/ArME14"
GENOME="$RUN_DIR/ArME14_v2_NCBI"
...
# run fastp trim
FASTP_TRIM=$(qsub -J1-$(cat $RUN_DIR/fastp_trim.cmds | wc -l) -l select=1:ncpus=12:mem=8GB,walltime=10:00:00 -N fastp_trim -v "CMDS_FILE=$RUN_DIR/fastp_trim.cmds","WORKDIR=$RUN_DIR" ${WORK_DIR}/array.pbspro | egrep -o "^[0-9]+")

# combine stderr files to log
cat *.e$FASTP_TRIM.* > fastp_trim.$FASTP_TRIM.log

# save failed commands to file
egrep "job killed|limit exceeded"  fastp_trim.e$FASTP_TRIM.* | awk -F [.:] '{print $3}' | awk 'NR==FNR{pos[$1]; next}FNR in pos' - $RUN_DIR/fastp_trim.cmds > $RUN_DIR/fastp_trim.cmds.$FASTP_TRIM.failed
# rerun failed jobs
echo "cd $RUN_DIR ; source ~/.bashrc; conda activate $CONDA_NAME; parallel $RUN_DIR/fastp_trim.cmds.$FASTP_TRIM.failed " | qsub -l select=1:ncpus=8:mem=16GB,walltime=12:00:00 -N repeat_fastp
...
# run for all files
ALIGN_BWA=$(qsub -J1-$(cat $RUN_DIR/align_reads.cmds | wc -l) -l select=1:ncpus=8:mem=12GB,walltime=4:00:00 -N align_reads -v "CMDS_FILE=$RUN_DIR/align_reads.cmds","WORKDIR=$RUN_DIR" ${WORK_DIR}/array.pbspro | egrep -o "^[0-9]+")
...
# merge and rename ONLY duplicated samples
gawk -F "\t" '$3 ~ /Ascochyta rabiei/ && $6 ~ /Illumina/' $WORK_DIR/WGS_sequencing_information_summary.txt | egrep -v "$IGNORE_SAMS"   | sort -k2 | gawk  -F "\t" -v ORS="" 'BEGIN{cmd="picard MergeSamFiles USE_THREADING=true QUIET=true COMPRESSION_LEVEL=0 O=/dev/stdout"}NR==1{printf "%s I=%s/%s.dedup.rg.csorted.bam", cmd, $1, $1; isolate=$2; split($6, a , " "); next}isolate!=$2{printf " | picard AddOrReplaceReadGroups SO=coordinate I=/dev/stdin CREATE_INDEX=true ID=%s_merged LB=%s_merged PL=Illumina PU=mixed SM=%s_merged O=%s_merged.csorted.bam\n%s I=%s/%s.dedup.rg.csorted.bam", isolate, isolate, isolate,  isolate, cmd, $1, $1; isolate=$2; split($6, a , " "); next}isolate==$2{printf " I=%s/%s.dedup.rg.csorted.bam", $1, $1}END{printf " | picard AddOrReplaceReadGroups SO=coordinate I=/dev/stdin CREATE_INDEX=true ID=%s_merged LB=%s_merged PL=Illumina PU=mixed SM=%s_merged O=%s_merged.csorted.bam\n", isolate, isolate,  isolate,  isolate}' | gawk 'NF>19' > $RUN_DIR/merge_rename_bams.cmds

# run for all files
MERGE_BAMS=$(qsub -J1-$(cat $RUN_DIR/merge_rename_bams.cmds | wc -l) -l select=1:ncpus=8:mem=16GB,walltime=1:30:00 -N merge_bams -v "CMDS_FILE=$RUN_DIR/merge_rename_bams.cmds","WORKDIR=$RUN_DIR" ${WORK_DIR}/array.pbspro | egrep -o "^[0-9]+")

# rename all individual bams
gawk -F "\t" '$3 ~ /Ascochyta rabiei/ && $6 ~ /Illumina/' $WORK_DIR/WGS_sequencing_information_summary.txt | egrep -v "$IGNORE_SAMS"  | sort -k2 | gawk  -F "\t" '{isolate=$2; split($6, a , " ");printf "picard AddOrReplaceReadGroups QUIET=true SO=coordinate I=%s/%s.dedup.rg.csorted.bam CREATE_INDEX=true ID=%s_%s LB=%s_%s PL=Illumina PU=%s SM=%s_%s CN=%s O=%s_%s.csorted.bam\n", $1, $1, isolate, $4, isolate, $4,  a[2], isolate, $4, $4, isolate, $4}' > $RUN_DIR/rename_bams.cmds
# there's an issue if there are 2 files for the same isolate from the same sequencing centre (like 11B and 11C from AgVic)

RENAME_BAMS=$(qsub -J1-$(cat $RUN_DIR/rename_bams.cmds | wc -l) -l select=1:ncpus=8:mem=16GB,walltime=1:30:00 -N rename_bams -v "CMDS_FILE=$RUN_DIR/rename_bams.cmds","WORKDIR=$RUN_DIR" ${WORK_DIR}/array.pbspro | egrep -o "^[0-9]+")

# find failed runs and repeat
cd $RUN_DIR
find . -maxdepth 1 -name "*.csorted.bam" -size 1M | cut -f 2 -d "/" | grep -f - rename_bams.cmds > rename_bams.cmds.failed
# repeat failed runs
echo "cd $RUN_DIR ; source ~/.bashrc; conda activate $CONDA_NAME; parallel rename_bams.cmds.failed" | qsub -l select=1:ncpus=8:mem=16GB,walltime=2:00:00 -N repeat_merge
...
# Qualimap bamqc
cd $RUN_DIR
ls -1 *_R1.fastq.gz  | egrep -v $IGNORE_RE | sort | parallel -k --dry-run --rpl "{file2} s:_R1:_R2:; uq()" --rpl "{files} s:_R1:_R?:; uq()" --rpl "{sample} s:_R1.+::"  "unset DISPLAY; cd  $RUN_DIR/{sample}; qualimap bamqc -bam {sample}.dedup.rg.csorted.bam --java-mem-size=4G -c -gff $GENOME.bed -outdir $RUN_DIR/{sample}/{sample}_bamqc" > $RUN_DIR/qualimap_bamqc.cmds
cd $RUN_DIR/pbs_logs
...
```{r snippy_stats, eval=TRUE, echo=FALSE, include=FALSE}
...
qualimap_files <- list.files(here("raw_data"), "multisampleBamQcReport.html", 
                             full.names = TRUE, 
                             recursive = TRUE)
...
qualimap_results <-  qualimap_files %>% .[grepl("/All", .)] %>% 
  map_df(~htmltab(., which = "//td/b[text() = 'Sample name']/ancestor::table")) %>% 
  rename(Submission_id=`Sample name`, Coverage=`Coverage mean`) %>% 
  dplyr::select(-one_of(exclude_columns)) %>% 
  mutate(across(-starts_with("Submission_id"), as.numeric)) %>%
  right_join(sequencing_table, .) %>% 
  relocate(Comments, .after = last_col()) %>% 
  relocate(Isolate, .before = 1) %>% 
  # left_join(., error_rates %>% dplyr::select(Isolate=replicate_group, Error_rate=allele_error_rate,
  #                                      Replicates=replicate_num)) %>%
  mutate(cloudstor_folder= glue::glue('<a href="{target}" target="_blank">{CloudStor_Folder}</a>')) %>% 
  select(-any_of(c("target", "CloudStor_Folder")))  %>% 
  arrange(Isolate, desc(Coverage)) %>% clean_names(case="snake", abbreviations=c("ID", "GC")) %>% 
  relocate(cloudstor_folder, .after = "sequencing_centre")
...
```{r map_stats, eval=TRUE, results='asis', echo=FALSE, out.width="100%"}
...
datatable(rownames = FALSE, colnames = snakecase::to_title_case(colnames(qualimap_results), abbreviations=c("ID", "GC")),
          extensions = "Buttons",
          # server = FALSE,
          options = list(paging = TRUE,
                           scrollX=TRUE, 
                           ...
                           buttons = c('copy', 'csv', 'excel')),
          escape = FALSE) %>% #, 
#  formatRound(~coverage+mapping_quality_mean, 1) %>% 
  formatPercentage("gc_percentage", 1) %>%
  formatString("coverage", suffix = "x") %>% 
          # options = list(dom = 'tf', pageLength = 40)) %>%
...
# qualimap_results %>% 
#               mutate(across(where(is.numeric), ~round(.x, digits = 1)),
#                 gc_percentage=gc_percentage/100) %>% clean_names(case="title", 
#                                                                  abbreviations = c("ID", "GC")) %>% download_this(. ,
#                 output_name = "Arab_2020_isolate_alignment_data",
#               output_extension = ".csv",
#               button_label = "Download table as csv file",
#               button_type = "success",
#               has_icon = TRUE,
...
BAM_DIR=$HOME/scratch/A_rabiei_WGS_2021/All_2018_2020_09_03_2023/
...
~/bin/split_ref_by_bai_datasize.py -s 1e6 -r $GENOME.fa.fai $BAM_DIR/AR0321_AGRF.csorted > $RUN_DIR/ArME14_target_1e6_regions_chr.bed
ln -s $RUN_DIR/ArME14_target_1e6_regions_chr.tsv $RUN_DIR/ArME14_target_1e6_regions_chr.bed
# prepare commands
BAM_FILES=$( find $BAM_DIR -maxdepth 1 -name "*.csorted.bam" -size +1M  | gawk -vORS=" " '1' )
cut -f1 $GENOME.fa.fai | parallel --dry-run "freebayes-parallel <(grep '{}' ArME14_target_regions_chr.tsv | gawk '{printf \"%s:%s-%s\n\", \$1, \$2, \$3}') \$NCPUS  -f $GENOME.fa -g 50000 -C3 -p1 $BAM_FILES > FB_array_output/{}.combined.vcf" > freebayes_g50k_array.cmds

cd $RUN_DIR/pbs_logs
# send to the cluster
FREEBAYES=$(qsub -J1-$(cat $RUN_DIR/freebayes_g50k_array.cmds | wc -l) -l select=1:ncpus=8:mem=48GB,walltime=18:00:00 -q sparks2 -N freebayes_array -v "CMDS_FILE=$RUN_DIR/freebayes_g50k_array.cmds","WORKDIR=$RUN_DIR" ${WORK_DIR}/array.pbspro) # -q sparks2
...
cat <(printf "file\ttotal_snps\tDP${MIN_DP}_filtered_snps\tQUAL20_filtered_snps\tQUAL30_filtered_snps\n") <(cat $RUN_DIR/FB_array_output/ArME14_ctg_*.vcf.stats) > vcf_stats_$(date +%d_%m_%Y).txt

# merge called variants
CONCAT_FB=$(echo "set -Eeo pipefail; source ~/.bashrc; conda activate $CONDA_NAME; cd $RUN_DIR ; cat $RUN_DIR/FB_array_output/ArME14_ctg_*.combined.vcf | vcffirstheader | vcfstreamsort -w 1000 | vcfuniq > A_rabiei_2018_2020_wgs_ArME14_v2.bwa.fb.vcf" | qsub -V -l select=1:ncpus=8:mem=16GB,walltime=20:00:00 -q sparks2 -N FB_concat | egrep -o "[0-9]+" )



# merge ind files to a single bam file (keep submission ids)
# BAMS=$( find $BAM_DIR -maxdepth 1 -name "*.csorted.bam" -size +1M  | gawk -v ORS=" " '{print "I="$1}' )
...
# run freebayes on BAMs (stringent)

cut -f1 $GENOME.fa.fai | parallel --dry-run "freebayes-parallel <(grep '{}' $RUN_DIR/ArME14_target_regions_chr.tsv | gawk '{printf \"%s:%s-%s\n\", \$1, \$2, \$3}') \$NCPUS  -f $GENOME.fa -p1 -g 50000 -0 $BAM_FILES > FB_array_output/{}.combined.stringent.vcf" > $RUN_DIR/freebayes_stringent.cmds
...
FREEBAYES_STRING=$(qsub -J1-$(cat $RUN_DIR/freebayes_stringent.cmds | wc -l) -l select=1:ncpus=8:mem=48GB,walltime=20:00:00 -N freebayes_string -q sparks2 -v "CMDS_FILE=$RUN_DIR/freebayes_stringent.cmds","WORKDIR=$RUN_DIR" ${WORK_DIR}/array.pbspro | egrep -o "^[0-9]+")
# save read groups to file
# sambamba view -H All_samples.csorted.combined.bam | grep '^@RG' > read_groups.txt


MIN_DP=5
ls -1 $RUN_DIR/FB_array_output/ArME14_ctg_*stringent.vcf  | parallel --dry-run "printf \"{}\t%s\t%s\t%s\t%s\n\" \$(cat {} | grep -c -v '^#') \$(cat {} | SnpSift filter \"( GEN[?].DP > $MIN_DP ) & ( GEN[?].GT != './.' )\" | gawk '{if (\$0 ~ /^#/ || (length(\$4)==1 && length(\$5)==1)); print \$0}' | grep -c -v '^#') \$(cat {} | SnpSift filter \"( GEN[?].DP > $MIN_DP ) & ( GEN[?].GT != './.' ) & ( QUAL > 20 )\" | gawk '{if (\$0 ~ /^#/ || (length(\$4)==1 && length(\$5)==1)); print \$0}' | grep -c -v '^#') \$(cat {} | SnpSift filter \"( GEN[?].DP > $MIN_DP ) & ( GEN[?].GT != './.' ) & ( QUAL > 30 )\" | gawk '{if (\$0 ~ /^#/ || (length(\$4)==1 && length(\$5)==1)); print \$0}' | grep -c -v '^#') > {}.stats" > $RUN_DIR/vcf_string_stats.cmds

cd $RUN_DIR/pbs_logs

# Calculate contig stats
VCF_STATS_STRING=$(qsub -J1-$(cat $RUN_DIR/vcf_stats.cmds | wc -l) -l select=1:ncpus=8:mem=16GB,walltime=2:00:00 -N vcf_stats -v"CMDS_FILE=$RUN_DIR/vcf_string_stats.cmds","WORKDIR=$RUN_DIR" ${WORK_DIR}/array.pbspro | egrep -o "^[0-9]+")


cat <(printf "file\ttotal_snps\tDP${MIN_DP}_filtered_snps\tQUAL20_filtered_snps\tQUAL30_filtered_snps\n") <(cat $RUN_DIR/FB_array_output/ArME14_ctg_*stringent.vcf.stats) > vcf_stats_stringent_$(date +%d_%m_%Y).txt

# filter the files and combine to a single VCF file, 
CONCAT_FB=$(echo "set -Eeo pipefail; source ~/.bashrc; conda activate $CONDA_NAME; cd $RUN_DIR ; cat $RUN_DIR/FB_array_output/ArME14_ctg_*.combined.stringent.vcf | vcffirstheader | vcfstreamsort -w 1000 | vcfuniq > A_rabiei_2018_2020.bwa.fb.stringent.vcf" | qsub -V -l select=1:ncpus=8:mem=16GB,walltime=20:00:00 -N FB_concat | egrep -o "[0-9]+" )
...
### Calling variants (using Octopus)

Calling variants with [Octopus](https://luntergroup.github.io/octopus) v0.7.4 [@cookeUnifiedHaplotypebasedMethod2021] was suggested to be highly accurate [@barbitoffSystematicBenchmarkStateoftheart2022] and was therefore also used to call variants from the read files.  `r fontFmt("Octopus")` was used to call the variants from the population of samples using a map-reduce strategy, following the recommended [guidelines](https://luntergroup.github.io/octopus/docs/guides/models/population).

```{bash octopus}
CONDA_NAME=/export/home/s2978925/miniconda3/envs/snippy
# install octopus
conda activate $CONDA_NAME
mamba install octopus distro seaborn
cd ~/etc/tools/SNP
git clone https://github.com/luntergroup/octopus.git
# set environment variables
WORK_DIR=$HOME/scratch/A_rabiei_WGS_2021
BAM_DIR="$WORK_DIR/All_2018_2020_09_03_2023"
REF_DIR=$HOME/data/reference_genomes/Ascochyta_reference_genomes/ArME14_v2_NCBI
GENOME="$REF_DIR/ArME14_v2_NCBI"
PLOIDY=1
DATE=`date +%d_%m_%Y`
RUN=Octopus_all_${DATE} # day of run was 02_02_2019
RUN_DIR=$WORK_DIR/${RUN}
mkdir -p $RUN_DIR
cd !$
# JOB_NAME="BBmap_me14_vars"
...
NCORES=8
cp $WORK_DIR/WGS_sequencing_information_summary.txt $RUN_DIR/
# edit manually to add B/C to duplicate isolates
# generate a profile of the input read data, which will be used to make new sequence error models.
# see which sequencing platforms were used
gawk -F "\t" '$3 ~ /Ascochyta rabiei/ && $6 ~ /Illumina/' $RUN_DIR/WGS_sequencing_information_summary.txt | cut -f6 | uniq
# find which samples use which technology
gawk -F "\t" '$3 ~ /Ascochyta rabiei/ && $6 ~ /Illumina/' $RUN_DIR/WGS_sequencing_information_summary.txt | grep -i nextseq
# generate a profile of the input read data, which will be used to make new sequence error models.
octopus --threads $NCORES -R $GENOME.fa -I $BAM_DIR/16CUR018_AGRF*.bam -o calls.bcf --data-profile NextSeq500_reads_profile.csv
~/etc/tools/SNP/profiler.py -P NextSeq500_reads_profile.csv -O NextSeq500.profile

octopus --threads $NCORES -R $GENOME.fa -I $BAM_DIR/16CUR018_merged*.bam -o calls.bcf --data-profile mixed_Illumina_reads_profile.csv
~/etc/tools/SNP/octopus/scripts/profiler.py -P mixed_Illumina_reads_profile.csv -O mixed_Illumina.profile
rm calls.bcf
# or submit to the cluster
OCTO_PROFILE=$(echo "set -Eeo pipefail; source ~/.bashrc; conda activate $CONDA_NAME; cd $RUN_DIR ; octopus --threads \$NCPUS -R $GENOME.fa -I $BAM_DIR/16CUR018_AGRF*.bam -o calls.bcf --data-profile NextSeq500_reads_profile.csv" | qsub -V -l select=1:ncpus=8:mem=16GB,walltime=2:00:00 -N Octopus_profile_concat | egrep -o "[0-9]+" )


# Call variants from individual samples
printf "AR0106\nAR0107\nAR0113\nAR0118\n11\n5B\n2\n15CUR005\nAGRF_18\nAGRF_08\nF15023\n20" > ignore_samples.txt
IGNORE_SAMS=$(awk '{printf "^%s\t|",$1}' $RUN_DIR/ignore_samples.txt | sed 's/|$//')
# For Macrogen Batch (HiSeq2500)
gawk -F "\t" '$3 ~ /Ascochyta rabiei/ && $6 ~ /HiSeq2500/' $RUN_DIR/WGS_sequencing_information_summary.txt | egrep -v "$IGNORE_SAMS"  | sort -k2 | gawk  -v ref="$GENOME.fa" -v bam_dir=$BAM_DIR -F "\t" '{isolate=$2; split($6, a , " ");printf "octopus --threads $NCPUS -R %s -I %s/%s_%s.csorted.bam -P 1 --sequence-error-model PCR.HISEQ-2500 -o %s_%s.bcf \n", ref, bam_dir, isolate, $4, isolate, $4}' > $RUN_DIR/octopus_calls_HiSeq2500.cmds

OCTO_HISEQ25=$(qsub -J1-$(cat $RUN_DIR/octopus_calls_HiSeq2500.cmds | wc -l) -l select=1:ncpus=8:mem=16GB,walltime=2:00:00 -N OCTO_HISEQ25 -v"CMDS_FILE=$RUN_DIR/octopus_calls_HiSeq2500.cmds","WORKDIR=$RUN_DIR" ${WORK_DIR}/array.pbspro | egrep -o "^[0-9]+")

# For AgVic Batch (HiSeq3000)
gawk -F "\t" '$3 ~ /Ascochyta rabiei/ && $6 ~ /HiSeq3000/' $RUN_DIR/WGS_sequencing_information_summary.txt | egrep -v "$IGNORE_SAMS"  | sort -k2 | gawk  -v ref="$GENOME.fa" -v bam_dir=$BAM_DIR -F "\t" '{isolate=$2; split($6, a , " ");printf "octopus --threads $NCPUS -R %s -I %s/%s*_%s.csorted.bam -P 1 --sequence-error-model PCR.HISEQ-4000 -o %s_%s.bcf \n", ref, bam_dir, isolate, $4, isolate, $4}' > $RUN_DIR/octopus_calls_HiSeq3000.cmds

# For AGRF Batch (NextSeq)
gawk -F "\t" '$3 ~ /Ascochyta rabiei/ && $6 ~ /NextSeq/' $RUN_DIR/WGS_sequencing_information_summary.txt | egrep -v "$IGNORE_SAMS"  | sort -k2 | gawk  -v ref="$GENOME.fa" -v bam_dir=$BAM_DIR -F "\t" '{isolate=$2; split($6, a , " ");printf "octopus --threads $NCPUS -R %s -I %s/%s_%s.csorted.bam -P 1 --sequence-error-model NextSeq500.profile -o %s_%s.bcf \n", ref, bam_dir, isolate, $4, isolate, $4}' > $RUN_DIR/octopus_calls_NextSeq.cmds


# For AGRF Batch (NovaSeq)
gawk -F "\t" '$3 ~ /Ascochyta rabiei/ && $6 ~ /NovaSeq/' $RUN_DIR/WGS_sequencing_information_summary.txt | egrep -v "$IGNORE_SAMS"  | sort -k2 | gawk  -v ref="$GENOME.fa" -v bam_dir=$BAM_DIR -F "\t" '{isolate=$2; split($6, a , " ");printf "octopus --threads $NCPUS -R %s -I %s/%s*_%s.csorted.bam -P 1 --sequence-error-model PCR.NOVASEQ -o %s_%s.bcf \n", ref, bam_dir, isolate, $4, isolate, $4}' > $RUN_DIR/octopus_calls_NovaSeq.cmds

# For merged samples Batch (mixed)
ls -1 $BAM_DIR/*merged.csorted.bam | sort | parallel -k --dry-run --rpl "{sample} s:.+/(.+?)_merged.csorted.bam:\1:"  "octopus --threads $NCPUS -R $GENOME.fa -I {} -P 1 --sequence-error-model mixed_Illumina.profile -o {sample}_merged.bcf" > $RUN_DIR/octopus_calls_merged_samples.cmds


# --rpl "{file2} s:_R1:_R2:; uq()" --rpl "{files} s:_R1:_R?:; uq()" --rpl "{sample} s:_R1.+::"  


gawk  -v ref="$GENOME.fa" -v bam_dir=$BAM_DIR -F "\t" '{isolate=$2; split($6, a , " ");printf "octopus --threads $NCPUS -R %s -I %s/%s*_%s.csorted.bam -P 1 --sequence-error-model PCR.NOVASEQ -o %s_%s.bcf \n", ref, bam_dir, isolate, $4, isolate, $4}' > $RUN_DIR/octopus_calls_NovaSeq.cmds

# BAMS=$(ls -1 $BAM_DIR/*.bam | awk -v ORS=" " '{print $1}')
ls -1 $BAM_DIR/*.bam > bam_files.txt

ls -1 $RUN_DIR/*.bcf > bcf_files.txt
# combine variants for a population with
mkdir Octo_pop_results
# split joint calling to batches of 20 samples at a time
seq 1 20 270 | parallel --dry-run sed -n \'{},'{=1 $_=$arg[1]+19 =}'p\' bam_files.txt \> bam_files{}.tmp \; sed -n \'{},'{=1 $_=$arg[1]+19 =}'p\' bcf_files.txt \> bcf_files{}.tmp \; octopus --threads $NCPUS -P 1 -C population -R $GENOME.fa -i bam_files{}.tmp --disable-denovo-variant-discovery --source-candidates-file bcf_files{}.tmp -o Octo_pop_results/All_Arabiei_WGS_samples_{}.octopus.joint.bcf > octopus_population.cmds
# run the commands
bash octopus_population.cmds
OCTO_POP_SET=$(qsub -J1-$(cat $RUN_DIR/octopus_population.cmds | wc -l) -l select=1:ncpus=8:mem=16GB,walltime=2:00:00 -N Octo_pop1 -v"CMDS_FILE=$RUN_DIR/octopus_population.cmds","WORKDIR=$RUN_DIR" ${WORK_DIR}/array.pbspro | egrep -o "^[0-9]+")

# final merge (this takes more than a week, maybe do in parallel by contig, like FB)
BCFS=$(ls -1 $RUN_DIR/Octo_pop_results/All_Arabiei_WGS_samples_*.octopus.joint.bcf | awk -v ORS=" " '{print $1}')

octopus --threads $NCPUS -P 1 -C population -R $GENOME.fa -i bam_files.txt --disable-denovo-variant-discovery -c $BCFS -o Octo_pop_results/All_Arabiei_WGS_samples.octopus.joint.bcf

# Call octopus on each chromosome
cut -f1 $GENOME.fa.fai | parallel --dry-run "octopus --threads \$NCPUS -P 1 -C population -T {} -R $GENOME.fa -i bam_files.txt --disable-denovo-variant-discovery -c $BCFS -o Octo_pop_results/All_Arabiei_WGS_samples.{}.octopus.joint.bcf" > $RUN_DIR/octopus_by_chr.cmds

OCTO_POP_FULL=$(echo "set -Eeo pipefail; source ~/.bashrc; conda activate $CONDA_NAME; cd $RUN_DIR ; octopus --threads \$NCPUS -P 1 -C population -R $GENOME.fa -i bam_files.txt --disable-denovo-variant-discovery -c $BCFS -o Octo_pop_results/All_Arabiei_WGS_samples.octopus.joint.bcf" | qsub -V -l select=1:ncpus=12:mem=32GB,walltime=12:00:00 -N Octopus_pop | egrep -o "[0-9]+" )
```


#### VCF filtering
Variants were filtered using a combination of commands from `r fontFmt("SnpSift")` v5.1d [@ruden_using_2012], `r fontFmt("BCFtools")` v1.17 [@danecekTwelveYearsSAMtools2021; @liStatisticalFrameworkSNP2011] and VCFtools v0.1.16 [@danecekVariantCallFormat2011a], based on their total loci depth, keeping only SNP loci with an average of 20 (per genotype) and not more than 100000 reads covering the locus (based on EDA). In addition, each isolate's genotype call was wiped (recoded as missing, or `./.`) if it had read depth (`DP<7`).
...
MIN_DP=10
IND_DP=7
# filter Freebayes variants (wipe any genotype with DP<7)
SnpSift filter "( QUAL>$QUAL ) & ( TYPE='snp' ) & ( DP<$MAX_DP ) & ( DP>$MIN_DP ) & ( countRef()>=1 & countVariant()>=1 )" A_rabiei_2018_2020.bwa.fb.stringent.vcf.gz | SnpSift gtfilter -gv './.' "( DP<$IND_DP )" > A_rabiei_2018_2020.bwa.fb.stringent.Q$QUAL.noRep.poly.vcf
# can also be performed with vcftools
vcftools --gzvcf A_rabiei_2018_2020.bwa.fb.stringent.vcf.gz --stdout --minDP $IND_DP --recode --recode-INFO-all --minQ 20 --max-missing 0.95 --remove duplicated_samples.csv --remove-indels | SnpSift filter "( QUAL>=$QUAL ) & ( DP<$MAX_DP ) & ( DP>$MIN_DP ) & ( countRef()>=1 & countVariant()>=1 )" >  Filtered/A_rabiei_2018_2020.bwa.fb.stringent.Q$QUAL.GT95.noRep.poly.vcf


bcftools filter -i 'F_PASS(DP>7 & GT!="mis") > 0.95 && QUAL>20 && TYPE="snp"' -O v | vcftools --vcf - 

# filter Octopus variants (no SNP tag)
SnpSift filter "( QUAL>$QUAL ) & ( DP<$MAX_DP ) & ( DP>$MIN_DP ) & ( countRef()>=1 & countVariant()>=1 )" All_Arabiei_WGS_samples.octopus.joint.vcf.gz | SnpSift gtfilter -gv './.' "( DP<$IND_DP )" > All_Arabiei_WGS_samples.octopus.joint.Q$QUAL.noRep.poly.vcf

# filter to keep only SNP loci with a maximum of 10% missing genotype calls
bcftools filter -e 'F_MISSING>0.1 && TYPE="snp"' All_Arabiei_WGS_samples.octopus.joint.Q20.noRep.poly.vcf > All_Arabiei_WGS_samples.octopus.joint.Q20.GT90.noRep.poly.vcf
...
An in-house R script (`estimate_error_rates_vcf_files.R`) was used to estimate the error rates based on the presence of duplicated samples. The variants called with `r fontFmt("Freebayes")` were found to be in an order of magnitude more accurate than the ones produced with `r fontFmt("Octopus")`.  

```{r estimate-error-vcf}
vcf_error_rates <- read_excel("output/variant_calling_pipeline_comparison.xlsx", 'pipeline_comparison_sum')
# produce table
vcf_error_rates %>% select(-`Replicate Num`) %>% 
  mutate(across(ends_with("Error Rate"), ~round(.x, digits = 4))) %>% 
  datatable(rownames = FALSE) %>% #, 
#  formatRound(~coverage+mapping_quality_mean, 1) %>% 
  formatPercentage(2:3, 2) 

```


#### Variant Join
The variants produced with TECAN were joined with the WGS ones (called with Freebayes) using `r fontFmt("BCFtools")` v1.17 [@danecekTwelveYearsSAMtools2021; @liStatisticalFrameworkSNP2011].

```{bash vcf-join}
bcftools merge --regions-overlap 2 -O v FB_new-genome_chr_joint-call.bcf.gz A_rabiei_2018_2020.bwa.fb.stringent.vcf.gz | bcftools filter -i 'F_PASS(DP>7 & GT!="mis") > 0.95' -O z > merged_TECAN_FB_WGS_snps.DP7-filtered.vcf.gz

SnpSift gtfilter -gv '.' "( DP<7 )" | bcftools view -O z -e 'F_MISSING>0.05 && QUAL>20' > merged_TECAN_FB_WGS_snps.DP7-filtered.vcf.gz

bcftools merge --regions-overlap 2 -O v FB_new-genome_chr_joint-call.bcf.gz A_rabiei_2018_2020.bwa.fb.stringent.vcf.gz | vcftools --vcf - --out merged_TECAN_FB_WGS_snps.DP7-filtered --minDP 7 --minQ 20 --recode --recode-INFO-all --max-missing 0.95 
# check how many SNPs remain
gzip -cd merged_TECAN_FB_WGS_snps.DP7-filtered.vcf.gz | grep -c -v "^#"
```
...
WORK_DIR=$PWD
GENOME_DIR="$HOME/data/reference_genomes/Ascochyta_reference_genomes/Updated_ME14_reference_and_annotations"
GENOME="Ascochyta_rabiei_ArME14.scaffolds.fa"
HECATON_HOME="$HOME/.nextflow/assets/IdoBar/hecaton"
FQ_DIR="$HOME/scratch/A_rabiei_WGS_2021/All_2018_2020_30_09_2021"
...
IGNORE_RE=$(awk '{printf "/%s_R|",$1}' ignore_samples.txt | sed 's/|$//')
# perform adapter trimming (fastp) 
ls -1 $FQ_DIR/AR0*.fastq.gz  | egrep -v $IGNORE_RE | sort | parallel -k ln -s {} fastq_dir/

# run Hecaton
conda activate nf
nextflow run -with-singularity  ~/aDNA/tools/.singularity/cache/hecaton_latest.sif -c ~/.nextflow/GU.config -profile gowonda -w hecaton_workdir $HOME/.nextflow/assets/IdoBar/hecaton/nextflow/hecaton.nf --genome_file $GENOME --reads "fastq_dir/*_R{1,2}.fastq.gz" --manta_config $HECATON_HOME/docker/configManta_weight_1.py.ini --output_dir hecaton_output --model_file $HECATON_HOME/models/random_forest_model_concat_A_thaliana_ColxCvi_O_sativa_Suijing18_coverage_10x_insertions_balanced_subsample.pkl -resume
...
# send the job to the cluster
HEC_MERGE_JOB=$( echo "source ~/.bashrc; conda activate hecaton; cd $PWD; set -Eeo pipefail; $HOME/.nextflow/assets/IdoBar/hecaton/scripts/genotype/merge_vcf_files.py -i AR_2020_vcfs.txt -f $HOME/scratch/A_rabiei_WGS_2021/Hecaton_all_05_07_2022/$GENOME.fai -o A_rabiei_2020_merged_samples.vcf -r 0.5 " | qsub -V -l select=1:ncpus=8:mem=4GB,walltime=1:00:00 -N cn_MOPS | egrep -o "^[0-9]+")
...
Alternatively, `r fontFmt("cn.MOPS")` can be run on the HPC by starting launching an interactive job and running the following in `r fontFmt("R")`:
...
`r fontFmt("CNVpytor")` (v2.7.2) is a Python pipeline to call CNVs from WGS data. [@suvakov2021]
...
CNVpytor=$(qsub -J1-$(cat cnvpytor.cmds | wc -l) -l select=1:ncpus=6:mem=8GB,walltime=5:00:00 -N cnvpytor -v "CMDS_FILE=$PWD/cnvpytor.cmds","WORKDIR=$PWD" ${PWD}/array.pbspro | egrep -o "^[0-9]+")
```

Find failed jobs and rerun. 
```{bash cnvpytor-rerun}
...
CNV_RERUN=$(qsub -J1-$(cat cnvpytor_rerun_failed.cmds | wc -l) -l select=1:ncpus=6:mem=8GB,walltime=5:00:00 -N cnvpytor -v "CMDS_FILE=$PWD/cnvpytor_rerun_failed.cmds","WORKDIR=$PWD" ${PWD}/array.pbspro | egrep -o "^[0-9]+")
```

Find failed jobs and rerun (2). 

```{bash cnvpytor-rerun2}
...
Merge calls from multiple samples - look at this [link](https://github.com/abyzovlab/CNVpytor#merging-calls-form-multiple-samples)
...
echo "cd $PWD; source ~/.bashrc'; conda activate $CONDA_NAME;  < merge_calls.spytor" | qsub -l select=1:ncpus=6:mem=8GB,walltime=5:00:00 -N cnvpytor_merge

```

Interactive analysis with CNVpytor
```{bash cnvpytor-interactive}



```

The data produced by CNVPytor was then exported to CloudStor (folder [jemma_cnv](https://cloudstor.aarnet.edu.au/plus/f/6815516595)) and visualised using a Jupyter notebook on [CloudStor SWAN](https://support.aarnet.edu.au/hc/en-us/articles/360000575395-CloudStor-SWAN). See explanation of output in [Predicting CNV regions](https://github.com/abyzovlab/CNVpytor/blob/master/GettingStarted.md#predicting-cnv-regions)

As an alternative to SWAN, the data can be analysed interactively by launching a Jupyter notebook directly from the Gowonda HPC (see documentation [here](https://griffith.atlassian.net/wiki/spaces/GHCD/pages/4030602/Jupyter+Notebook))
```{bash cnvpytor-jupyter}
WORK_DIR=$HOME/aDNA/jemma_cnv
cd $WORK_DIR
...
qsub -l select=1:ncpus=6:mem=8GB,walltime=600:00:00 cnvpytor_jupyter.pbs
```
...
## Dual RNA-Seq Read Alignment
```{bash dualrnaseq-bbsplit}
# bbsplit (for RNA-Seq)
# create index
bbsplit.sh build=1 ref_chickpea=$HOME/data/reference_genomes/Cicer_arietinum/Car.genome.fa.gz ref_rabiei=$HOME/data/reference_genomes/Ascochyta_reference_genomes/ArME14_v2_NCBI/ArME14_v2_NCBI.fa threads=\$[NCPUS - 2]

ls -1 ~/aDNA/Mumu_RNA-seq/AGRF_CAGRF220911960_H5L5CDSX7_genozip/downsampled_files/*_R1.fastq.gz | parallel -k --dry-run --rpl "{infile} s:_R1:_R#:; uq()" --rpl "{sample} s:.+/(.+?)_H5L5CDSX7_[ACGT\-]+_L004_.+:\1:"  "bbsplit.sh build=1 threads=\$[NCPUS - 2] in={infile} basename={sample}_%_R#.fastq.gz refstats={sample}.bbsplit.stats maxindel=100k ambiguous2=split qtrim=lr" > bbsplit.cmds

BBSPLIT=$(qsub -J1-$(cat bbsplit.cmds | wc -l) -l select=1:ncpus=10:mem=16GB,walltime=2:00:00 -N bbsplit_RNA-Seq -q workq -v"CMDS_FILE=bbsplit.cmds","WORKDIR=$PWD" array.pbspro | egrep -o "^[0-9]+")

# collate stats
head -n1 sample_25-L11.bbsplit.stats | paste <(echo "sample")  - > stats.headers

ls -1 sample*.stats | parallel --rpl "{sample} s:.bbsplit.stats::" 'tail -n +2 {} | paste <(echo -e "{sample}\n{sample}") -' | cat stats.headers - > all_samples.combined.stats
