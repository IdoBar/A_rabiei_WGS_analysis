...
qualimap_results <-  qualimap_files %>% map_df(~htmltab(., which = "//td/b[text() = 'Sample name']/ancestor::table")) %>% 
  ...
  write_tsv(filedate(glue::glue("{analysis_basename}_AGRF_Macro_AgVic.mapping.stats"), ".txt", "./output/results", dateformat = FALSE))
...
mapping_summary <- qualimap_results %>% filter(Coverage>20) %>% 
  ...
  write_tsv(filedate(glue::glue("{analysis_basename}_AGRF_Macro_AgVic.mapping.sum"), ".txt", "./output/results", dateformat = FALSE))
...
BAMS=$( ls -1 $MERGE_JOB/*.csorted.bam | gawk -v ORS="" 'NR==1; NR>1{printf ","$1}' )
SAMPLES=$( echo $BAMS | sed "s|$MERGE_JOB/||g; s/_bbmap_me14.csorted.bam//g" )

# Call variants (needs lots of memory, but very quick)
echo "cd $( pwd ); callvariants.sh in=$BAMS sample=$SAMPLES maf=0.02 rarity=0.02 minreads=5 multisample ploidy=$PLOIDY ow border=20 out=A_rabiei_2018_isolates_Macro_AGRF_AgriVic_haplo.var vcf=A_rabiei_2018_isolates_Macro_AGRF_AgriVic_haplo.vcf ref=$GENOME.fasta"  | qsub -V -l select=1:ncpus=12:mem=96GB,walltime=10:00:00 -N bb_callvars -m e -M i.bar@griffith.edu.au # 5248656.pbsserver
...
gawk 'NR==FNR{s[$2]++;next} (s[$2]>1)'  <(egrep -v $IGNORE_SAMS ../A_rabiei_isolate_list_for_wgs.txt ) <(egrep -v $IGNORE_SAMS ../A_rabiei_isolate_list_for_wgs.txt) > duplicated_samples.txt
DUP_BAMS=$( gawk '{printf "%s_bbmap_me14.csorted.bam,",$1}' duplicated_samples.txt | sed 's/,$//' )
DUP_SAMPLES=$( gawk '{printf "%s_%s,",$2,NR}' duplicated_samples.txt | sed 's/,$//' )

# Call variants (needs lots of memory, but very quick)
echo "cd $( pwd ); callvariants.sh in=$DUP_BAMS sample=$DUP_SAMPLES maf=0.02 rarity=0.02 minreads=5 multisample ploidy=$PLOIDY border=20 out=A_rabiei_2018_dup_samples_haplo.var vcf=A_rabiei_2018_dup_samples_haplo.vcf ref=$GENOME.fasta"  | qsub -V -l select=1:ncpus=12:mem=96GB,walltime=10:00:00 -N bb_calldups -m e -M i.bar@griffith.edu.au # 5248654.pbsserver
# find and remove empty files
find . -size 0 -exec rm {} + 
rm -r *.csorted.bam $MERGE_JOB
...
# Fix names 
SnpSift filter "( DP>5 ) & ( FILTER='PASS' ) & ( TYP='SUB' )" A_rabiei_2018_isolates_Macro_AGRF_AgriVic_haplo.vcf | SnpSift gtfilter -gv './.' "( DP<5 ) | ( DP>500 ) "   > A_rabiei_2018_isolates_Macro_AGRF_AgriVic_haplo.DP.passed.snps.vcf
# filter only SNPs
