...
cloudstor_links_2018 <- hyperlinks_from_excel(here("sample_info/A_rabiei_isolate_list_for_wgs.xlsx"), 
                    ...
                    select(target, CloudStor_Folder=text) %>% 
                    distinct()
...
sequencing_table <- sequencing_table_2020 %>% 
  select(Submission_id=`Sample Name`, Isolate, CloudStor_Folder, target) %>% 
...
DT_samples_table <- samples_table %>% 
  select(-any_of(c("Sample_Id", "Isolate_Code", "Supplied_by", "Delivered_to", "Collection_Date",
...
Calling variants with [Octopus](https://luntergroup.github.io/octopus) (citation from https://www.nature.com/articles/s41587-021-00861-3) is suggested to be highly accurate (citation from https://bmcgenomics.biomedcentral.com/articles/10.1186/s12864-022-08365-3#Sec9).  
See these [guidelines](https://luntergroup.github.io/octopus/docs/guides/models/population) to call variants from a population
...
As an alternative to SWAN, the data can be analysed interactively bu launching a Jupyter notebook directly from the Gowonda HPC (see documentation [here](https://griffith.atlassian.net/wiki/spaces/GHCD/pages/4030602/Jupyter+Notebook))
