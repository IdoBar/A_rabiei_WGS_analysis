...
COMBINE_ID=$( echo "cd $( pwd ); ls -1 ../*_R1.fastq.gz | egrep -v \"F15023\" | sort | xargs cat > raw_R1.fq.gz; ls -1 ../*_R2.fastq.gz | egrep -v \"F15023\" | sort | xargs cat > raw_R2.fq.gz" | qsub -V -l select=1:ncpus=12:mem=8GB,walltime=20:00 -N combine_reads )
...
COMBINE_ID=$( echo $COMBINE_ID | awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}' )

# Create the index, trim adapters, mapp and calculate quality matrices
RECALC_ID=$( echo "cd $( pwd ); bbduk.sh in=raw_R#.fq.gz out=stdout.fq ref=$BBMAP_DIR/resources/adapters.fa ktrim=r k=23 mink=11 hdist=1 minlen=$READ_LENGTH tpe tbo int=f | bbmap.sh in=stdin.fq out=raw.sam ref=$GENOME.fasta ambig=toss slow int && calctruequality.sh in=raw.sam ref=$GENOME.fasta callvariants ploidy=$PLOIDY" | qsub -V -l select=1:ncpus=12:mem=96GB,walltime=10:00:00 -N recalc_qual -W depend=afterok:$COMBINE_ID ) # 5247555.pbsserver
...
QUALIMAP_JOB_ID=$( echo "cd $( pwd ) ; unset DISPLAY ; ls -1 *.csorted.bam | gawk '{match(\$0, /(.+)_bbmap_me14.csorted.bam/, a); printf \"%s\\t%s\\n\",a[1], \$0 }' > $QUALIMAP_JOB.samples ; qualimap multi-bamqc -r -d $QUALIMAP_JOB.samples -outformat PDF:HTML -outdir $QUALIMAP_JOB -outfile $QUALIMAP_JOB.pdf" | qsub -V -l select=1:ncpus=12:mem=4GB,walltime=1:00:00 -N ${QUALIMAP_JOB:0:11} -W depend=afterok:$JOB_ID[] ) # 5248673.pbsserver
QUALIMAP_JOB_ID=$( echo $QUALIMAP_JOB_ID | awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}' )

# multiqc report
MULTIQC_JOB="${BATCH}_bb_qc_${DATE}"
...
# Check that all QC jobs finished successfuly
...
MERGE_LANES_ID=$( qsub -J1-$MERGE_NUM -N ${MERGE_LANES:0:11} -vCMDS_FILE=$MERGE_LANES.cmds | awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}' )

# Combine all _1 and _2 reads (ignoring contaminated/low quality samples)
COMBINE_ID=$( echo "cd $( pwd ); ls -1 ./*_R1.fastq.gz | egrep -v 'AGRF_05|AGRF_08|AGRF_18' | sort | xargs cat > raw_R1.fq.gz ; ls -1 ./*_R2.fastq.gz | egrep -v 'AGRF_05|AGRF_08|AGRF_18' | sort | xargs cat > raw_R2.fq.gz" | qsub -V -l select=1:ncpus=12:mem=8GB,walltime=20:00 -N combine_reads )
COMBINE_ID=$( echo $COMBINE_ID | awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}' )


# Create the index, trim adapters, mapp and calculate quality matrices
RECALC_ID=$( echo "cd $( pwd ); bbduk.sh in=raw_R#.fq.gz out=stdout.fq ref=$BBMAP_DIR/resources/adapters.fa ktrim=r k=23 mink=11 hdist=1 minlen=$READ_LENGTH tpe tbo int=f | bbmap.sh in=stdin.fq out=raw.sam ref=$GENOME.fasta ambig=toss slow int ; calctruequality.sh in=raw.sam ref=$GENOME.fasta callvariants ploidy=$PLOIDY" | qsub -V -l select=1:ncpus=12:mem=96GB,walltime=10:00:00 -N recalc_qual -W depend=afterok:$COMBINE_ID )
RECALC_ID=$( echo $RECALC_ID | awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}' ) # 5248486.pbsserver

# Then perform adapter trimming, mapping, clumping, recalibration
find ./ -maxdepth 1 -name "*_R1.fastq.gz" | sort | gawk -v bbmap_dir=$BBMAP_DIR -v genome=$GENOME -v RGPL=$RGPL -v RGPU=$RGPU -v RGCN=$RGCN -v CLUMP_PARAM="$CLUMP_PARAM" 'BEGIN{command=sprintf("bbduk.sh -Xmx1g ref=%s/resources/adapters.fa ktrim=r k=23 mink=11 hdist=1 qtrim=rl trimq=10 tpe tbo int minlen=30 ziplevel=9 ow", bbmap_dir)};{n=split($1,a,"/"); infile=gensub("_R1\\.", "_R#.", "1", $1); basename=gensub(/(.+)_R1.fastq.gz/, "\\1", "1", a[n]) ; m=split(basename,b,"_") ;sample=b[1]"_"b[2]; printf("cd $PBS_O_WORKDIR; filterbytile.sh in=%s out=stdout.fq int=f | clumpify.sh in=stdin.fq out=stdout.fq int dedupe optical %s adjacent | bbduk.sh in=stdin.fq out=stdout.fq int recalibrate |  %s in=stdin.fq out=recal_trimmed_%s_R#.fastq.gz stats=%s.stats ow && bbmap.sh in=recal_trimmed_%s_R#.fastq.gz outm=stdout.sam outu=%s_bbmap_me14.unmapped.fq.gz machineout local ow int=f rgsm=%s rgid=%s_%s rgpl=%s rgpu=%s rgcn=%s pigz unpigz slow k=12 | picard SortSam I=/dev/stdin O=%s_bbmap_me14.csorted.bam SO=coordinate \n",infile, CLUMP_PARAM, command, basename, basename, basename, basename, sample, sample, NR, RGPL,RGPU,RGCN, sample)}' > ${RUN}.bash
...
# fix read file names
my_rename -v 's/_001//; s/HWYNNBBXX_L003//' *.fastq.gz
# Prepare the BBduk commands
BATCH=agvic
DATE=`date +%d_%m_%Y`
RUN="${BATCH}_BB_process_${DATE}" # day of run was 02_02_2019
...
analysis_folder <- "var_data/BB_vars_26_02_2019"
...
ln -s ~/scratch/data/A_rabiei_WGS/AgVic_WGS2018/agvic_BB_process_22_02_2019/*.csorted.bam ./
ln -s ~/scratch/data/A_rabiei_WGS/AGRF_CAGRF19461_HGMLMAFXY/AGRF_BB_process_22_02_2019/*.csorted.bam ./
ln -s ~/data/A_rabiei_sequencing/Macrogen_sequences_1702KHP-0164/macrogen_BB_process_22_02_2019/*.csorted.bam ./
...
# merge sam files of duplicated samples and rename to isolate name
 egrep -v $IGNORE_SAMS $HOME/scratch/data/A_rabiei_WGS/A_rabiei_isolate_list_for_wgs.txt | tail -n+2 | sort -k2 | gawk  -v ORS="" 'BEGIN{cmd="cd $PBS_O_WORKDIR; picard MergeSamFiles QUIET=true COMPRESSION_LEVEL=0 O=/dev/stdout"}NR==1{printf "%s I=%s_bbmap_me14.csorted.bam", cmd, $1; isolate=$2; next}isolate!=$2{printf " | picard AddOrReplaceReadGroups I=/dev/stdin ID=%s LB=%s PL=Illumina PU=combined SM=%s O=%s.csorted.bam\n%s I=%s_bbmap_me14.csorted.bam", isolate, isolate, isolate,  isolate, cmd, $1; isolate=$2; next}isolate==$2{printf " I=%s_bbmap_me14.csorted.bam", $1}END{printf " | picard AddOrReplaceReadGroups I=/dev/stdin ID=%s LB=%s PL=Illumina PU=mixed SM=%s O=%s.csorted.bam\n", isolate, isolate, isolate,  isolate}'  > $MERGE_JOB.cmds
 
# Prepare PBS script
echo '#!'"/bin/bash -v
#PBS -V
#PBS -l select=1:ncpus=${ncpus}:mem=64GB,walltime=01:30:00
"'
cd $PBS_O_WORKDIR
...
FINAL_MERGE=$( echo "cd $( pwd ) ; sambamba merge -t $ncpus all.csorted.combined.bam $BAM_FILES" | qsub -V -l select=1:ncpus=${ncpus}:mem=96GB,walltime=5:00:00 -N Final_merge -W depend=afterok:$MERGE_JOB_ID[] ) # 5248672.pbsserver
FINAL_MERGE_ID=$( echo $FINAL_MERGE | awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}')

# save read groups to file
sambamba view -H all.csorted.combined.bam | grep "^@RG" > all_read_groups.txt
# BAM_FILES=$( ls -1 $MERGE_JOB/*.csorted.bam | gawk -vORS=" " '1' )
# calculate coverage
CALC_COV=$( echo "cd $( pwd ) ;  sambamba depth base --combined all.csorted.combined.bam | cut -f 1-3 | tail -n +2 | coverage_to_regions.py $GENOME.fasta.fai 500 > targets.regions " | qsub -V -l select=1:ncpus=${ncpus}:mem=96GB,walltime=5:00:00 -N Calc_cov -W depend=afterok:$FINAL_MERGE_ID ) # 5248672.pbsserver
CALC_COV_ID=$( echo $CALC_COV | awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}')
# /export/home/s2978925/.pyenv/versions/miniconda2-latest/bin/bamtools coverage -in
# prepare Freebayes commands
FB=freebayes_${DATE}
FB_CMDS=$( echo "cd $( pwd ) ; cat targets.regions | parallel --dry-run -k \"freebayes -p $PLOIDY -f $GENOME.fasta all.csorted.combined.bam --region {}\" > $FB.cmds" | qsub -V -l select=1:ncpus=${ncpus}:mem=96GB,walltime=5:00:00 -N FB_cmds -W depend=afterok:$CALC_COV_ID ) # 5248672.pbsserver
# get job_id
FB_CMDS_ID=$( echo $FB_CMDS | awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}')

# Send freebayes jobs to the cluster
# Prepare PBS script
echo '#!'"/bin/bash -v
#PBS -V
#PBS -l select=1:ncpus=${ncpus}:mem=64GB,walltime=20:00:00
"'
cd $PBS_O_WORKDIR
ARRAYID=$( echo $PBS_ARRAY_ID | egrep -o "[0-9]{7}" )
gawk -v ARRAY_IND=$PBS_ARRAY_INDEX'" 'NR>=(${ncpus}*3*(ARRAY_IND-1)) && NR<=(ARRAY_IND*${ncpus}*3)' \${CMDS_FILE} | parallel -k -j ${ncpus} --joblog"' ${CMDS_FILE}.p$ARRAYID.$PBS_ARRAY_INDEX' > freebayes.pbspro

NCMDS=$( wc -l $FB.cmds | gawk '{print $1}')
NJOBS=$(( NCMDS/ncpus/3 + 1  ))
...
FB_JOB_ID=$( echo $FB_JOB | awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}')

CONCAT_FB=$( echo "cd $( pwd ) ; cat *.o$FB_JOB_ID* | vcffirstheader | vcfstreamsort -w 1000 | vcfuniq > A_rabiei_2018_isolates_haplo.fb.vcf" | qsub -V -l select=1:ncpus=${ncpus}:mem=96GB,walltime=10:00:00 -N FB_vcf -W depend=afterok:$FB_JOB_ID[] -m e -M i.bar@griffith.edu.au)
CONCAT_FB_ID=$( echo $CONCAT_FB | egrep -o "[0-9]{7}" )

# check jobs completion
grep "ExitStatus"  *.e* # | cat -d ":" -f2,2 | uniq
...
It is advised to process each bam separately as well to be able to estimate error rates and expected relatedness between duplicate samples.
...
# link all sam files in the same folder
ln -s ~/scratch/data/A_rabiei_WGS/AgVic_WGS2018/agvic_BB_process_22_02_2019/*.csorted.bam ./ind_bams/
ln -s ~/scratch/data/A_rabiei_WGS/AGRF_CAGRF19461_HGMLMAFXY/AGRF_BB_process_22_02_2019/*.csorted.bam ./ind_bams/
ln -s ~/data/A_rabiei_sequencing/Macrogen_sequences_1702KHP-0164/macrogen_BB_process_22_02_2019/*.csorted.bam ./ind_bams/
# remove lane information
my_rename -v 's/_L00[1-9]//' *L00*.bam
MERGE_JOB="merge_bams"
...
FINAL_MERGE=$( echo "cd $( pwd ) ; sambamba merge -t $ncpus all_ind.csorted.combined.bam $BAM_FILES" | qsub -V -l select=1:ncpus=${ncpus}:mem=96GB,walltime=5:00:00 -N Final_merge ) # 5248672.pbsserver
FINAL_MERGE_ID=$( echo $FINAL_MERGE | awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}')

# save read groups to file
sambamba view -H all_ind.csorted.combined.bam | grep "^@RG" > ind_read_groups.txt
# BAM_FILES=$( ls -1 $MERGE_JOB/*.csorted.bam | gawk -vORS=" " '1' )

# calculate coverage
CALC_COV=$( echo "cd $( pwd ) ;  sambamba depth base --combined all_ind.csorted.combined.bam | cut -f 1-3 | tail -n +2 | coverage_to_regions.py $GENOME.fasta.fai 500 > targets.regions " | qsub -V -l select=1:ncpus=${ncpus}:mem=96GB,walltime=5:00:00 -N Calc_cov -W depend=afterok:$FINAL_MERGE_ID ) # 5248672.pbsserver
CALC_COV_ID=$( echo $CALC_COV | awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}')
# /export/home/s2978925/.pyenv/versions/miniconda2-latest/bin/bamtools coverage -in
# prepare Freebayes commands
FB=freebayes_${DATE}
FB_CMDS=$( echo "cd $( pwd ) ; cat targets.regions | parallel --dry-run -k \"freebayes -p $PLOIDY -f $GENOME.fasta all_ind.csorted.combined.bam --region {}\" > $FB.cmds" | qsub -V -l select=1:ncpus=${ncpus}:mem=96GB,walltime=5:00:00 -N FB_cmds -W depend=afterok:$CALC_COV_ID ) # 5248672.pbsserver
# get job_id
FB_CMDS_ID=$( echo $FB_CMDS | awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}')
...
CONCAT_FB=$( echo "cd $( pwd ) ; cat *.o$FB_JOB_ID* | vcffirstheader | vcfstreamsort -w 1000 | vcfuniq > A_rabiei_2018_isolates_haplo.ind.fb.vcf" | qsub -V -l select=1:ncpus=${ncpus}:mem=96GB,walltime=10:00:00 -N FB_vcf -W depend=afterok:$FB_JOB_ID[] -m e -M i.bar@griffith.edu.au)
CONCAT_FB_ID=$( echo $CONCAT_FB | egrep -o "[0-9]{7}" )
...
FB_STATS=$( echo "cd $( pwd ) ; mkdir ${FB}_stats; bcftools stats -F $GENOME.fasta -s $SAMPLES A_rabiei_2018_isolates_haplo.ind.fb.vcf > ${FB}_stats/A_rabiei_2018_isolates_haplo.ind.fb.vcf.stats; plot-vcfstats -p ${FB}_stats ${FB}_stats/A_rabiei_2018_isolates_haplo.ind.fb.vcf.stats" | qsub -V -l select=1:ncpus=${ncpus}:mem=96GB,walltime=10:00:00 -N FB_vcfstats -W depend=afterok:$CONCAT_FB_ID -m e -M i.bar@griffith.edu.au)
FB_STATS_ID=$( echo $FB_STATS | egrep -o "[0-9]{7}" )
...
#### VCF filtering
Variants were filtered using `r fontFmt("SnpSift")` v4.3.1 [@ruden_using_2012], based on their total loci depth, keeping ony SNP loci with at least 100 reads covering the locus (DP$\ge100$) and not more than 2000 (based on EDA). In addition, each isolate genotype call was removed (recoded as `./.`) if it had read depth <5.
```{bash vcf_filter}
...
QUAL=20
DP=20000
SnpSift filter "( QUAL>$QUAL ) & ( TYPE='snp' ) & ( DP<$DP ) & ( countRef()>=1 & countVariant()>=1 )" A_rabiei_2018_isolates_haplo.ind.fb.vcf > A_rabiei_2018_isolates_haplo.ind.Qual${QUAL}.U${DP}DP.poly.snps.vcf

SnpSift filter "( QUAL>$QUAL ) & ( TYPE='snp' ) & ( DP<$DP ) & ( countRef()>=1 & countVariant()>=1 )" A_rabiei_2018_isolates_haplo.fb.vcf > A_rabiei_2018_isolates_haplo.Qual${QUAL}.U${DP}DP.poly.snps.vcf
...
Association between variants and pathogenicity levels was identified by `r fontFmt("SNPassoc")` R package [@gonzalez_snpassoc:_2007], using a codominant gene model and a significance threshold of _p_-value $\le0.005$.  
