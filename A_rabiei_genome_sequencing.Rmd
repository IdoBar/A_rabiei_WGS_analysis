...
FQ_STATS=$(  find `pwd` -name "*.f*q.gz" | parallel --dry-run "printf '{/.}\t{=s/.+_(R[12]).fastq.gz/\1/=}\t%s\n' \$(( \$(pigz -cd {} | wc -l)/4 ))" | qsub -V -l select=1:ncpus=${ncpus}:mem=96GB,walltime=5:00:00 -N read_stats )
...
#### VCF filtering
Variants were filtered using `r fontFmt("SnpSift")` v4.3.1 [@ruden_using_2012], based on their total loci depth, keeping ony SNP loci with at least 100 reads covering the locus (DP$\ge100$) and not more than 20000 (based on EDA). In addition, each isolate genotype call was removed (recoded as `./.`) if it had read depth <5.
```{bash vcf_filter}
