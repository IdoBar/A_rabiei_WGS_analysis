...
The sequencing data of 2 isolates with sufficient coverage and no contamination (Macrogen batch DON001, AgriVic batch 7_S40) were analyzed for their k-mer content (coverage and distribution) to estimate the genome size and ploidy of the genome.
k-mer counting and genome ploidy and size estimation were performed with `kmercountexact.sh` (from BBMap v38.34, see [SeqAnsewes thread](http://seqanswers.com/forums/showthread.php?t=64086)).  
...
cat ${SAMPLE}_peaks_k21.txt | grep "^#" | head -n -1 | sed "s/#//" | cat <( printf "sample\t%s\n" $SAMPLE) - | cut -f1 | paste -s > kmer_peaks_summary.txt

SAMPLES=(DON001 7_S40 AGRF_11)
for SAMPLE in "${SAMPLES[@]}"; do 
  ls -1 ${SAMPLE}_khist*.txt | parallel "tail -n +2 {} | gawk -v sample=$SAMPLE '{printf "%s\t$0", sample}'" >> kmer_peaks_summary.txt
done
# do the same for the hist data (maybe with gawk)
 gawk 'NR==1{printf "sample\tkmer\t%s\n", $0}' ${SAMPLE}_khist_k21.txt >  khist_summary.txt
