...
~/bin/netcheck
conda install -n snippy -c conda-forge -c bioconda snippy bcbio-gff libgd xorg-libxpm libpng  libjpeg-turbo jpeg libtiff
# Clean extra space
conda clean -y --all
...
The trimmed reads were mapped to the _A. rabiei_ reference genome, strain Me14 ([GCA_004011695.1](https://www.ncbi.nlm.nih.gov/assembly/GCA_004011695.1)) using `r fontFmt("Snippy")` v4.4.0. `r fontFmt("Snippy")` is a wrapper for popular bioinformatics tools, using `r fontFmt("bwa-mem")` to map the reads to the reference genome, followed by several commands of `r fontFmt("samtools")`, to specify a Read Group for each sample (provided to Snippy with the `--rgid` flag), mark duplicates and converted into coordinate-sorted, indexed BAM.  
...
find ./ -maxdepth 1 -name "*_R1.fastq.gz" | sort | gawk -F"\t" -v genome=$GENOME.gb -v ncores=$NCORES '{n=split($1,a,"/"); read2=gensub("_R1\\.", "_R2.", "1", $1); basename=gensub(/trimmed_(.+)_R1.fastq.gz/, "\\1", "1", a[n]) ; printf("cd $PBS_O_WORKDIR; source ~/.bashrc; conda activate snippy; snippy --cpus %s --outdir %s --report --rgid %s --ref %s --R1 %s --R2 %s \n", ncores,  basename, basename, genome, $1, read2)}' > ${RUN}.bash
...
# run Qualimap
QUALIMAP_JOB_ID=$( echo "cd $( pwd ) ; unset DISPLAY ; qualimap multi-bamqc -r -d $QUALIMAP_JOB.samples -outformat PDF:HTML -outdir $QUALIMAP_JOB -outfile $QUALIMAP_JOB.pdf" | qsub -V -l select=1:ncpus=12:mem=4GB,walltime=1:00:00 -N ${QUALIMAP_JOB:0:11} -W depend=afterok:$SNPY_CORE_ID) # 5248673.pbsserver
...
find . -name "*.bam" | gawk '{n=split($0, a, "/"); printf "%s\t%s\n",a[2], $0}' > $QUALIMAP_JOB.samples
QUALIMAP_JOB_ID=$( echo "cd $( pwd ) ; unset DISPLAY ; qualimap multi-bamqc -r -d $QUALIMAP_JOB.samples -outformat PDF:HTML -outdir $QUALIMAP_JOB -outfile $QUALIMAP_JOB.pdf" | qsub -V -l select=1:ncpus=12:mem=4GB,walltime=1:00:00 -N ${QUALIMAP_JOB:0:11} -W depend=afterok:$BT2_ID[] ) # 5248673.pbsserver
QUALIMAP_JOB_ID=$( echo $QUALIMAP_JOB_ID | egrep -o "[0-9]{7}" )
