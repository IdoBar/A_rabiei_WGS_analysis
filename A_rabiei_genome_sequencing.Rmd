...
The trimmed reads were mapped to the _A. rabiei_ reference genome, strain _Me14_ (Rob Lee, Curtin University) using `r fontFmt("Bowtie2")` (v2.3.4.3) with the `--very-sensitive` option in `--end-to-end` (global) mode. The resulting SAM files were processed to mark duplicates with `r fontFmt("SAMBLASTER")` v0.1.24 [@faust_samblaster:_2014], add read group information and convert into indexed, 

The resulting SAM files were sorted and compressed to BAM format with [Picard v2.18.22](https://broadinstitute.github.io/picard/). Post-trimming and mapping statistics were consolidated into a single, interactive report using `r fontFmt("MultiQC")` v0.9.1 [@ewels_multiqc:_2016]
...
mapping_files <- dir("./raw_data", ".stats", full.names = TRUE)


mapping_table <- mapping_files %>% map_dfr(read_tsv(.) %>% rename(Isolate=sample_id) %>%
  mutate_at(vars(contains("mapped")),  ~as.numeric(sub("%", "", ., fixed=TRUE))/100) %>%
  mutate(Mapping_rate=rowMeans(.[,-1]), Mapping_tool = "BBmap", 
         Isolate=str_split(Isolate, "_") %>% map_chr(~paste(.[1:2], collapse = "_"))) %>%
  group_by(Isolate, Mapping_tool) %>%
  summarise(Mapping_rate=mean(Mapping_rate)) %>% ungroup() %>% select(Isolate, Mapping_rate, Mapping_tool)) 
...
ln -s ~/scratch/data/A_rabiei_WGS/AgVic_WGS2018/BB_process_02_02_2019/*_bbmap_me14.csorted.bam ./
