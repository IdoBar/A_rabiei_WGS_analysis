...
The resulting kmer count data for all samples and kmers was aggregated into tables and summarised in the following plot.  
...
``` {r kmer_plot, eval=TRUE, echo=FALSE}
...
# plot for each kmer
ggplot(plot_hist, mapping = aes(x=Depth, y=Count, colour=factor(kmer))) + scale_y_continuous(labels = scales::comma) +
  geom_line(size=1) + scale_color_paletteer_d(awtools, mpalette) + # RColorBrewer, Set1
  facet_wrap(vars(Sample)) + plot_theme() + labs(colour="K-mer")
ggsave("output/")
