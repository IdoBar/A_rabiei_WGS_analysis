...
pca_data <- pca1$li %>% rownames_to_column("Submission_id")  %>%
  # inner_join(sequencing_table) %>% 
...
outliers <- pca_data %>% inner_join(replicated_samples) %>% .[,"Isolate_head"]
...
p <- ggplot(pca_data, aes(x=Axis1, y=Axis2, fill=Sequencing_Centre, 
                          # size=Year,
...
p + geom_point(alpha = 0.85, size=5) + # , stroke = 1
  ...
  geom_text_repel(aes(label=Isolate_head, colour=adjustcolor( c("dodgerblue3"), alpha.f = 0.8)),
  ...
  guides(shape = guide_legend(override.aes = list(size = 5 , fill=pal[1:3]), order = 1, title = "Seq.centre"), #
         fill = "none") +   #  guide_legend(override.aes = list(size = 5, pch=shapes[1]), order = 2)
...
ggsave(filedate(glue::glue("{analysis_basename}_Ind_PCA_seq_centre_PC1-2"),
                ext = ".pdf", outdir = glue::glue("{analysis_outdir}/plots"),
                dateformat = FALSE), width = 10, height=8)
save.image(filedate(glue("{analysis_basename}_duplicate_analysis"), ext = ".RData", glue("{analysis_outdir}")))
